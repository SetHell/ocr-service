...
def confianza_zona(zon, nom):
    if not zon:
        return 0.0
    items = zon.get(nom, {}).get("items", [])
    if not items:
        return 0.0
    return max(i.get("confianza", 0) for i in items)

UMBRAL_CONFIANZA = 0.45

def zona_vacia(zon, nom):
    txt = texto_zona(zon, nom).strip()
    if not txt:
        return True
    conf = confianza_zona(zon, nom)
...
def extraer_placa(textos, zonas):
    return extraer_placa_zona(zonas) or extraer_placa_general(textos)

BASURA_IMPRESA = [
    "SEÑOR", "SENOR", "CONDUCTOR", "PARTIR", "PAGO", "INFRACCION",
    "ENTIDADES", "FINANCIERAS", "GRACIAS", "CONTRIBUIR", "SIGUIENTES",
    "HORAS", "UNICAMENTE", "BANCOS", "PLURINACIONAL", "BOLIVIA",
    "BOLETA", "INFRACCION", "POLICIA", "BOLIVIANA", "DIRECCION",
    "FISCALIZACION", "RECAUDACIONES", "INFORMACIONES", "TALONARIOS",
    "SERIE", "FUNCIONARIO", "POLICIAL", "NOMBRE", "APELLIDO",
]

def es_basura_impresa(txt: str):
    txt_l = limpiar(txt)
    return any(b in txt_l for b in BASURA_IMPRESA)

def texto_manuscrito_valido(txt: str, min_chars=2):
    txt = limpiar(txt).strip()
    if not txt or len(txt) < min_chars:
        return None
    if es_basura_impresa(txt):
        return None
    return txt

def extraer_no_porta(txt: str):
    claves = ["NO PORTA", "NO POR", "NO PORLC", "NO PORTC", "NO PORTE", "SIN LICENCIA"]
    return any(clave in txt for clave in claves)

def extraer_licencia(txt: str, zonas=None):
    if zonas and zona_vacia(zonas, "licencia"):
        return None
...
def extraer_nombre_conductor(textos):
    ...
    canddts = []
    EXCLUIR = {
        "DATOS DEL VEHICULO", "TIPO", "MARCA", "COLOR",
        "LICENCIA", "PLACA", "HORA", "DIA", "MES", "ANO",
    }
    for item in textos:
        txt = limpiar(item["texto"])
        cx = centro_x(item)
        ...
        if abs(cy - cy_ancla) < 70 and cx > ax2 and len(txt) > 2:
            if txt not in EXCLUIR and not es_basura_impresa(txt):
                canddts.append((txt, cx, item["confianza"]))
    if not canddts:
        return None
    canddts.sort(key=lambda x: x[1])
    nom = " ".join(x[0] for x in canddts)
    nom = texto_manuscrito_valido(nom, min_chars=5)
    return nom
...
def normalizar_respuesta(textos, caslls, zonas=None):
    ...
    datos = {
        ...
        "placa": placa,
        "nro_lic": extraer_licencia(txt, zonas),
    ...
    camps_rev = []
    if not datos["hora"]: camps_rev.append("HORA")
    if not datos["dia"]: camps_rev.append("DÍA")
    if not datos["mes"]: camps_rev.append("MES")
    if not datos["anio"]: camps_rev.append("AÑO")
    if not placa_valida(datos["placa"]): camps_rev.append("PLACA")
    if not datos["tip_vehi"]: camps_rev.append("TIPO VEHÍCULO")
    if not datos["marca"]: camps_rev.append("MARCA")
    if not datos["color"]: camps_rev.append("COLOR")
    if not articulo_valido(datos["art"]): camps_rev.append("ARTÍCULO")
    if not num_valido(datos["nro_infr"]): camps_rev.append("NÚMERO INFRACCIÓN")
    if not datos["lugar"]: camps_rev.append("LUGAR")
    if not datos["zona"]: camps_rev.append("ZONA")
